...
<Steps>
  ...
  <Step title="Configure group settings">
    ...
    ```bash  theme={null}
    asc testflight groups update --id "GROUP_ID" \
      --public-link-enabled \
    ...
    ```bash  theme={null}
    asc testflight groups update --id "GROUP_ID" --feedback-enabled
    ```
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="List and filter testers">
    ...
    ```bash  theme={null}
    asc testflight testers list --app "YOUR_APP_ID" --build "BUILD_ID"
    ```
  </Step>
